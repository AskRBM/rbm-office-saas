...
# =====================================================================
# PHASE 25 ENTERPRISE EXTENSION
# Consolidated enterprise-grade scaffolding: advanced masters, approval
# center, soft delete, audit, utilities, and expanded reports.
# =====================================================================

# ---------- ENTERPRISE TABLE REGISTRATION ----------
ENTERPRISE_TABLES = {
    "roles": "roles",
...
TABLES.update(ENTERPRISE_TABLES)

ENTERPRISE_DISPLAY_COLUMNS = {
    "roles": ["id","client_code","role_name","role_level","status","created_by","created_at"],
    "permissions": ["id","client_code","role_name","module_name","can_view","can_create","can_edit","can_delete","can_approve","can_reverse","status","created_by","created_at"],
    "client_features": ["id","client_code","feature_name","is_enabled","remarks","created_by","created_at"],
    "branches": ["id","client_code","branch_name","branch_code","address","state","gst_no","status","created_by","created_at"],
    "departments": ["id","client_code","department_name","department_code","branch_name","status","created_by","created_at"],
    "warehouses": ["id","client_code","warehouse_name","warehouse_code","branch_name","address","status","created_by","created_at"],
    "bank_accounts": ["id","client_code","bank_name","account_name","account_no","ifsc_code","branch_name","ledger_name","opening_balance","status","created_by","created_at"],
    "hsn_sac_master": ["id","client_code","hsn_sac_code","description","gst_rate","type","status","created_by","created_at"],
    "tax_masters": ["id","client_code","tax_name","tax_type","tax_rate","ledger_name","status","created_by","created_at"],
    "tds_sections": ["id","client_code","section_code","section_name","tds_rate","threshold_amount","ledger_name","status","created_by","created_at"],
    "payment_terms": ["id","client_code","term_name","credit_days","discount_percent","remarks","status","created_by","created_at"],
    "approval_roles": ["id","client_code","module_name","maker_role","checker_role","approver_role","amount_limit","status","created_by","created_at"],
    "voucher_types": ["id","client_code","voucher_type","module_name","numbering_prefix","next_no","status","created_by","created_at"],
    "opening_balances": ["id","client_code","financial_year","ledger_name","ledger_group","opening_dr","opening_cr","remarks","created_by","created_at"],
    "opening_stock": ["id","client_code","financial_year","warehouse_name","item_name","stock_group","unit","opening_qty","opening_rate","opening_value","remarks","created_by","created_at"],
    "approval_logs": ["id","client_code","module_name","record_id","old_status","new_status","approval_remarks","approved_by","approved_at","created_at"],
    "login_history": ["id","client_code","username","login_time","status","ip_address","device_info","created_at"],
    "failed_login_history": ["id","client_code","username","attempt_time","reason","ip_address","device_info","created_at"],
    "export_history": ["id","client_code","report_name","export_type","rows_count","exported_by","created_at"],
    "pdf_history": ["id","client_code","document_type","document_no","pdf_name","generated_by","created_at"],
    "stock_ledger": ["id","client_code","transaction_date","voucher_type","voucher_no","warehouse_name","item_name","in_qty","out_qty","rate","value","remarks","created_by","created_at"],
    "bank_reconciliation": ["id","client_code","bank_ledger","transaction_date","particulars","book_amount","bank_amount","reco_status","remarks","created_by","created_at"],
}
DISPLAY_COLUMNS.update(ENTERPRISE_DISPLAY_COLUMNS)

# Add universal enterprise columns to transactional tables where available.
ENTERPRISE_CONTROL_COLUMNS = [
    "branch_name", "department", "warehouse_name", "financial_year", "document_status",
    "submitted_by", "submitted_at", "checked_by", "checked_at",
    "approved_date", "updated_by", "updated_at", "is_deleted", "deleted_by", "deleted_at",
    "sync_status", "online_id", "uuid_key"
]
for _k in REVERSIBLE_TABLE_KEYS + ["ledgers", "stock_ledgers"]:
    if _k in DISPLAY_COLUMNS:
        for _c in ENTERPRISE_CONTROL_COLUMNS:
            if _c not in DISPLAY_COLUMNS[_k]:
                DISPLAY_COLUMNS[_k].append(_c)

# ---------- AUDIT / SAFE DML OVERRIDES ----------
def write_audit_log(module_name, action_type, record_id="", details=""):
    """Write audit trail. This never stops the main transaction if audit logging fails."""
    try:
        if "audit_logs" not in TABLES:
...
def log_approval(module_name, record_id, old_status, new_status, remarks=""):
    try:
        supabase.table(TABLES["approval_logs"]).insert({
            "client_code": get_client_code(),
            "module_name": module_name,
            "record_id": str(record_id),
            "old_status": str(old_status or ""),
            "new_status": str(new_status or ""),
            "approval_remarks": str(remarks or ""),
            "approved_by": current_user(),
            "approved_at": india_now().isoformat(),
        }).execute()
    except Exception:
        pass
...
def insert_row(key, row):
    if key != "clients" and "client_code" not in row:
        row["client_code"] = get_client_code()
    row = dict(row)
    row.setdefault("created_by", current_user())
    row.setdefault("sync_status", "Online")
    row.setdefault("is_deleted", False)
    try:
        row.setdefault("uuid_key", f"RBM-{india_now().strftime('%Y%m%d%H%M%S%f')}")
    except Exception:
        pass
    response = supabase.table(TABLES[key]).insert(row).execute()
    try:
        new_id = response.data[0].get("id", "") if response.data else ""
        if key != "audit_logs":
            write_audit_log(key, "CREATE", new_id, f"Created record in {key}")
    except Exception:
        pass
...
def update_row(key, row_id, row):
    clean = clean_update_row(row)
    clean["updated_by"] = current_user()
    clean["updated_at"] = india_now().isoformat()
    response = supabase.table(TABLES[key]).update(clean).eq("id", int(row_id)).execute()
    write_audit_log(key, "UPDATE", row_id, f"Updated record in {key}")
    return response


def delete_row(key, row_id):
    """Enterprise rule: prefer soft delete. Fallback to hard delete only if table has no columns."""
    try:
        response = supabase.table(TABLES[key]).update({
            "is_deleted": True,
            "deleted_by": current_user(),
            "deleted_at": india_now().isoformat(),
            "document_status": "Deleted",
        }).eq("id", int(row_id)).execute()
        write_audit_log(key, "SOFT_DELETE", row_id, f"Soft deleted record in {key}")
        return response
    except Exception:
        response = supabase.table(TABLES[key]).delete().eq("id", int(row_id)).execute()
        write_audit_log(key, "DELETE", row_id, f"Hard deleted record in {key}")
        return response

# ---------- GENERIC ENTERPRISE MASTERS ----------
def enterprise_master_form(table_key, title, fields, section="section-master"):
    show_header(title, section)
    st.caption("Enterprise master data. Records are client-wise and audit logged.")
    with st.form(f"form_{table_key}"):
        values = {}
        cols = st.columns(2)
        for i, f in enumerate(fields):
            ...
            c = cols[i % 2]
            if typ == "select":
                values[name] = c.selectbox(label, options or ["Active", "Inactive"], key=f"{table_key}_{name}")
            elif typ == "number":
                values[name] = c.number_input(label, value=float(default or 0), key=f"{table_key}_{name}")
            elif typ == "date":
                values[name] = c.date_input(label, value=india_now().date(), format="DD-MM-YYYY", key=f"{table_key}_{name}").isoformat()
            elif typ == "bool":
                values[name] = c.checkbox(label, value=bool(default), key=f"{table_key}_{name}")
            elif typ == "textarea":
                values[name] = st.text_area(label, value=str(default or ""), key=f"{table_key}_{name}")
            else:
                values[name] = c.text_input(label, value=str(default or ""), key=f"{table_key}_{name}")
        if st.form_submit_button(f"Save {title}", use_container_width=True):
            insert_row(table_key, values)
            st.success(f"{title} saved successfully.")
            st.rerun()
    show_table_with_edit_delete(table_key, load_table(table_key, 2000), f"{title} List")


def role_permission_master():
    enterprise_master_form("permissions", "Role & Permission Master", [
        {"name":"role_name"}, {"name":"module_name"},
        {"name":"can_view", "type":"bool", "default":True}, {"name":"can_create", "type":"bool", "default":True},
        {"name":"can_edit", "type":"bool", "default":True}, {"name":"can_delete", "type":"bool", "default":False},
        {"name":"can_approve", "type":"bool", "default":False}, {"name":"can_reverse", "type":"bool", "default":False},
        {"name":"status", "type":"select", "options":["Active","Inactive"]},
    ])


def branch_master():
    enterprise_master_form("branches", "Branch / Location Master", [
        {"name":"branch_name"},{"name":"branch_code"},{"name":"address","type":"textarea"},{"name":"state"},{"name":"gst_no"},{"name":"status","type":"select","options":["Active","Inactive"]}
    ])


def department_master():
    branches = ["None"] + (load_table("branches", 1000)["branch_name"].dropna().astype(str).tolist() if "branches" in TABLES else [])
    enterprise_master_form("departments", "Department Master", [
        {"name":"department_name"},{"name":"department_code"},{"name":"branch_name","type":"select","options":branches},{"name":"status","type":"select","options":["Active","Inactive"]}
    ])


def warehouse_master():
    branches = ["None"] + (load_table("branches", 1000)["branch_name"].dropna().astype(str).tolist() if "branches" in TABLES else [])
    enterprise_master_form("warehouses", "Warehouse / Godown Master", [
        {"name":"warehouse_name"},{"name":"warehouse_code"},{"name":"branch_name","type":"select","options":branches},{"name":"address","type":"textarea"},{"name":"status","type":"select","options":["Active","Inactive"]}
    ])


def bank_master():
    enterprise_master_form("bank_accounts", "Bank Master", [
        {"name":"bank_name"},{"name":"account_name"},{"name":"account_no"},{"name":"ifsc_code"},{"name":"branch_name"},{"name":"ledger_name","type":"select","options":get_ledger_names("Bank Accounts")},{"name":"opening_balance","type":"number"},{"name":"status","type":"select","options":["Active","Inactive"]}
    ])


def hsn_sac_master():
    enterprise_master_form("hsn_sac_master", "HSN / SAC Master", [
        {"name":"hsn_sac_code"},{"name":"description"},{"name":"gst_rate","type":"number","default":18},{"name":"type","type":"select","options":["HSN","SAC"]},{"name":"status","type":"select","options":["Active","Inactive"]}
    ])


def tax_master():
    enterprise_master_form("tax_masters", "GST / Tax Master", [
        {"name":"tax_name"},{"name":"tax_type","type":"select","options":["CGST","SGST","IGST","CESS","TDS","TCS","Other"]},{"name":"tax_rate","type":"number"},{"name":"ledger_name","type":"select","options":get_ledger_names("Duties & Taxes")},{"name":"status","type":"select","options":["Active","Inactive"]}
    ])


def tds_section_master():
    enterprise_master_form("tds_sections", "TDS Section Master", [
        {"name":"section_code"},{"name":"section_name"},{"name":"tds_rate","type":"number"},{"name":"threshold_amount","type":"number"},{"name":"ledger_name","type":"select","options":get_ledger_names()},{"name":"status","type":"select","options":["Active","Inactive"]}
    ])


def payment_terms_master():
    enterprise_master_form("payment_terms", "Payment Terms Master", [
        {"name":"term_name"},{"name":"credit_days","type":"number"},{"name":"discount_percent","type":"number"},{"name":"remarks","type":"textarea"},{"name":"status","type":"select","options":["Active","Inactive"]}
    ])


def approval_role_master():
    enterprise_master_form("approval_roles", "Approval Role Master", [
        {"name":"module_name"},{"name":"maker_role"},{"name":"checker_role"},{"name":"approver_role"},{"name":"amount_limit","type":"number"},{"name":"status","type":"select","options":["Active","Inactive"]}
    ])


def voucher_type_master():
    enterprise_master_form("voucher_types", "Voucher Type Master", [
        {"name":"voucher_type"},{"name":"module_name"},{"name":"numbering_prefix"},{"name":"next_no","type":"number","default":1},{"name":"status","type":"select","options":["Active","Inactive"]}
    ])


def opening_balance_master():
    enterprise_master_form("opening_balances", "Opening Balance Master", [
        {"name":"financial_year","default":financial_year(india_now().date())},{"name":"ledger_name","type":"select","options":get_ledger_names()},{"name":"ledger_group","type":"select","options":["All"]+get_groups("ledger_groups","group_name",DEFAULT_LEDGER_GROUPS)},{"name":"opening_dr","type":"number"},{"name":"opening_cr","type":"number"},{"name":"remarks","type":"textarea"}
    ])


def opening_stock_master():
    warehouses = ["Main"] + (load_table("warehouses", 1000)["warehouse_name"].dropna().astype(str).tolist() if "warehouses" in TABLES else [])
    enterprise_master_form("opening_stock", "Opening Stock Master", [
        {"name":"financial_year","default":financial_year(india_now().date())},{"name":"warehouse_name","type":"select","options":warehouses},{"name":"item_name","type":"select","options":get_stock_items()},{"name":"stock_group","type":"select","options":["All"]+get_groups("stock_groups","stock_group_name",DEFAULT_STOCK_GROUPS)},{"name":"unit"},{"name":"opening_qty","type":"number"},{"name":"opening_rate","type":"number"},{"name":"opening_value","type":"number"},{"name":"remarks","type":"textarea"}
    ])

# ---------- APPROVAL / CONTROL CENTER ----------
def approval_center():
    show_header("Approval Center", "section-rep")
    st.info("Enterprise rule: Draft entries do not affect management reports. Approve/Post before final reporting.")
    modules = ["sales","purchase","expenses","service_vouchers","stock_vouchers","fixed_assets","accounting_entries","appointments","tasks","attendance","inout","visitors"]
    selected = st.selectbox("Select Module", modules)
    df = load_table(selected, 5000)
    if df.empty:
        st.info("No entries found.")
        return
    status_filter = st.selectbox("Approval Status", ["All", "Pending", "Approved", "Rejected", "Not Required"])
    if status_filter != "All" and "approval_status" in df.columns:
        df = df[df["approval_status"].astype(str) == status_filter]
    st.dataframe(df, use_container_width=True)
    if not df.empty and st.session_state.get("role") in ["Admin", "Super Admin"]:
        selected_id = st.selectbox("Select Entry ID", df["id"].tolist(), key="approval_selected_id")
        c1, c2, c3 = st.columns(3)
        remarks = st.text_input("Approval / Rejection Remarks")
        if c1.button("✅ Approve", use_container_width=True):
            old = str(df[df["id"] == selected_id].iloc[0].get("approval_status", ""))
            update_row(selected, selected_id, {"approval_status":"Approved", "approved_by":current_user(), "approved_date":india_now().isoformat(), "approval_remarks":remarks})
            log_approval(selected, selected_id, old, "Approved", remarks)
            st.success("Entry approved."); st.rerun()
        if c2.button("📌 Post", use_container_width=True):
            update_row(selected, selected_id, {"parking_status":"Posted", "document_status":"Posted"})
            write_audit_log(selected, "POST", selected_id, "Entry posted")
            st.success("Entry posted."); st.rerun()
        if c3.button("❌ Reject", use_container_width=True):
            old = str(df[df["id"] == selected_id].iloc[0].get("approval_status", ""))
            update_row(selected, selected_id, {"approval_status":"Rejected", "approval_remarks":remarks})
            log_approval(selected, selected_id, old, "Rejected", remarks)
            st.warning("Entry rejected."); st.rerun()

# ---------- ENTERPRISE UTILITIES ----------
def enterprise_import_export_center():
    show_header("Utilities / Import / Export / Backup", "section-rep")
    tabs = st.tabs(["Excel Import", "Google Sheet", "Full Backup", "Offline Sync Notes"])
    with tabs[0]:
        st.subheader("Mass Excel / CSV Import")
        table_keys = [k for k in TABLES.keys() if k not in ["clients"] or is_super_admin()]
        target = st.selectbox("Target Table", table_keys, key="enterprise_import_target")
        uploaded = st.file_uploader("Upload Excel / CSV", type=["xlsx", "csv"], key="enterprise_upload")
        if uploaded is not None:
            try:
                df = pd.read_csv(uploaded) if uploaded.name.lower().endswith(".csv") else pd.read_excel(uploaded)
                st.dataframe(df.head(100), use_container_width=True)
                if st.button("Import Rows", use_container_width=True):
                    success = 0
                    for _, row in df.iterrows():
                        insert_row(target, {k: (None if pd.isna(v) else v) for k, v in row.to_dict().items()})
                        success += 1
                    insert_row("import_logs", {"import_type":"Excel/CSV", "module_name":target, "total_rows":len(df), "success_rows":success, "failed_rows":len(df)-success, "remarks":"Enterprise import", "created_by":current_user()})
                    st.success(f"Imported {success} rows.")
            except Exception as e:
                st.error(f"Import failed: {e}")
    with tabs[1]:
        st.subheader("Google Sheet Import")
        url = st.text_input("Paste published Google Sheet CSV URL")
        target = st.selectbox("Target Table", [k for k in TABLES.keys()], key="gsheet_target")
        if st.button("Preview Google Sheet") and url:
            try:
                df = pd.read_csv(url)
                st.dataframe(df.head(100), use_container_width=True)
            except Exception as e:
                st.error(f"Could not read sheet: {e}")
        st.caption("For private Google Sheets, use Google API/service account in the next enterprise phase.")
    with tabs[2]:
        st.subheader("Full Data Backup")
        if st.button("Prepare Excel Backup", use_container_width=True):
            output = BytesIO()
            with pd.ExcelWriter(output, engine="openpyxl") as writer:
                for k in TABLES.keys():
                    try:
                        ...
                        pass
            st.download_button("Download Full Backup Excel", output.getvalue(), "rbm_erp_full_backup.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    with tabs[3]:
        st.info("Online + Offline architecture: Online Supabase/PostgreSQL, Offline SQLite, future sync using uuid_key, updated_at, sync_status, online_id and conflict resolution.")

# ---------- ENHANCED REPORTS ----------
_core_reports = reports

def enterprise_reports():
    show_header("Enterprise Reports", "section-rep")
    tab0, tab1, tab2, tab3, tab4 = st.tabs(["Core Reports", "GST / TDS", "Ageing", "Bank Reconciliation", "Export History"])
    with tab0:
        _core_reports()
    with tab1:
        st.subheader("GST Register")
        gst_rows = []
        for key, date_col, party_col in [("sales","invoice_date","customer_name"),("purchase","invoice_date","supplier_name"),("expenses","expense_date","vendor_name"),("service_vouchers","voucher_date","customer_name")]:
            try:
                df = load_table(key, 50000)
                if not df.empty:
                    for _, r in df.iterrows():
                        gst_rows.append({"Module":key,"Date":r.get(date_col,""),"Party":r.get(party_col,""),"Taxable":r.get("taxable_value",0),"CGST":r.get("cgst",0),"SGST":r.get("sgst",0),"IGST":r.get("igst",0),"Total":r.get("total_value",0)})
            except Exception:
                pass
        gst_df = pd.DataFrame(gst_rows)
        st.dataframe(gst_df, use_container_width=True)
        st.download_button("Download GST Register", to_excel_bytes(gst_df), "gst_register.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        st.subheader("TDS Summary")
        tds_rows = []
        for key in ["purchase", "expenses"]:
            df = load_table(key, 50000)
            if not df.empty and "tds" in df.columns:
                tmp = df[[c for c in ["invoice_date","expense_date","supplier_name","vendor_name","tds","total_value"] if c in df.columns]].copy()
                tmp["Module"] = key
                tds_rows.append(tmp)
        tds_df = pd.concat(tds_rows, ignore_index=True) if tds_rows else pd.DataFrame()
        st.dataframe(tds_df, use_container_width=True)
    with tab2:
        st.subheader("Receivable / Payable Ageing")
        tb = build_trial_balance_df()
        if tb.empty:
            st.info("No ledger data found.")
        else:
            ageing = tb[tb["ledger_group"].astype(str).isin(["Sundry Debtors","Sundry Creditors"])].copy()
            ageing["Balance"] = ageing.get("closing_dr",0).astype(float) - ageing.get("closing_cr",0).astype(float)
            st.dataframe(ageing[["ledger_group","ledger_name","email","contact_no","Balance"]], use_container_width=True)
    with tab3:
        st.subheader("Bank Reconciliation")
        enterprise_master_form("bank_reconciliation", "Bank Reconciliation", [
            {"name":"bank_ledger","type":"select","options":get_ledger_names("Bank Accounts")},{"name":"transaction_date","type":"date"},{"name":"particulars"},{"name":"book_amount","type":"number"},{"name":"bank_amount","type":"number"},{"name":"reco_status","type":"select","options":["Pending","Matched","Difference"]},{"name":"remarks","type":"textarea"}
        ], "section-rep")
    with tab4:
        for key in ["import_logs","export_history","pdf_history","audit_logs","approval_logs"]:
            st.subheader(key.replace("_"," ").title())
            try:
                st.dataframe(load_table(key, 1000), use_container_width=True)
            except Exception:
                st.info(f"{key} table not ready.")

# ---------- UPDATED MENUS / MAPPING ----------
def get_menu_modules(group):
    modules = []
    if group == "Dashboard":
        modules = ["Dashboard"]
        ...
        modules = [
            "Company Profile", "Financial Year Master", "Branch / Location Master", "Department Master", "Cost Center Master",
            "Role & Permission Master", "Approval Role Master", "Document Series", "Voucher Type Master",
            "Ledger Group Master", "Ledger Master", "Opening Balance Master", "Bank Master",
            "Stock Group Master", "Stock Ledger Master", "Opening Stock Master", "Warehouse Master",
            "HSN / SAC Master", "GST Settings", "GST / Tax Master", "TDS Section Master", "Payment Terms Master"
        ] if is_super_admin() or st.session_state.get("allow_master_group", False) else []
    elif group == "Admin":
        ...
        if st.session_state.get("allow_appointment", False) or is_super_admin(): modules += ["Appointments"]
    elif group == "HR":
        if st.session_state.get("allow_attendance", False) or is_super_admin(): modules.append("Attendance Management")
        if st.session_state.get("allow_inout", False) or is_super_admin(): modules.append("IN / OUT Register")
        if st.session_state.get("allow_visitor", False) or is_super_admin(): modules.append("Visitor Register")
        if st.session_state.get("allow_task", False) or is_super_admin(): modules.append("Task Delegation")
    elif group == "Inventory":
        if st.session_state.get("allow_stock_raw", False) or is_super_admin(): modules.append("Raw Material Stock")
        if st.session_state.get("allow_stock_fg", False) or is_super_admin(): modules.append("Finished Goods Stock")
        if st.session_state.get("allow_stock_wip", False) or is_super_admin(): modules.append("WIP Stock")
        if modules or is_super_admin(): modules += ["Stock Voucher", "Stock Ledger Report"]
    elif group == "Accounts":
        if st.session_state.get("allow_sales", False) or is_super_admin(): modules.append("Sales GST Invoice")
        if st.session_state.get("allow_purchase", False) or is_super_admin(): modules.append("Purchase GST Invoice")
        if st.session_state.get("allow_expense", False) or is_super_admin(): modules.append("Expense GST")
        if st.session_state.get("allow_service_voucher", False) or is_super_admin(): modules.append("Service Voucher")
        if st.session_state.get("allow_fixed_assets", False) or is_super_admin(): modules.append("Fixed Assets")
        if st.session_state.get("allow_accounting", False) or is_super_admin(): modules.append("Accounting Entries")
    elif group == "Quotation":
        if is_super_admin() or st.session_state.get("allow_quotation", False) or _quote_role(): modules = ["Quotation"]
    elif group == "Approval":
        if is_super_admin() or st.session_state.get("role") == "Admin": modules = ["Approval Center", "ERP Control Center"]
    elif group == "Reports":
        modules = ["Enterprise Reports", "Registers / Reports", "Audit Log"]
    elif group == "Utilities":
        modules = ["Import / Export / Backup", "Calculation Book"]
    elif group == "Settings":
        modules = ["Company Profile", "Feature Control Notes"]
    return modules or ["No module available"]


def build_group_list():
    if _quote_role(): return ["Quotation"]
    if is_super_admin(): return ["Dashboard","Master","Admin","HR","Inventory","Accounts","Quotation","Approval","Reports","Utilities","Settings"]
    groups = []
    has_any = any([st.session_state.get(k, False) for k in ["allow_master_group","allow_attendance","allow_inout","allow_visitor","allow_task","allow_appointment","allow_stock_raw","allow_stock_fg","allow_stock_wip","allow_sales","allow_purchase","allow_expense","allow_service_voucher","allow_fixed_assets","allow_accounting","allow_excel_upload","allow_google_sheet_import"]])
    if has_any: groups.append("Dashboard")
    if st.session_state.get("allow_master_group", False): groups.append("Master")
    if st.session_state.get("role") == "Admin" or st.session_state.get("allow_appointment", False): groups.append("Admin")
    if any([st.session_state.get(k, False) for k in ["allow_attendance","allow_inout","allow_visitor","allow_task"]]): groups.append("HR")
    if any([st.session_state.get(k, False) for k in ["allow_stock_raw","allow_stock_fg","allow_stock_wip"]]): groups.append("Inventory")
    if any([st.session_state.get(k, False) for k in ["allow_sales","allow_purchase","allow_expense","allow_service_voucher","allow_fixed_assets","allow_accounting"]]): groups.append("Accounts")
    if st.session_state.get("allow_quotation", False): groups.append("Quotation")
    if st.session_state.get("role") == "Admin": groups.append("Approval")
    if has_any: groups.append("Reports")
    if st.session_state.get("allow_excel_upload", False) or st.session_state.get("allow_google_sheet_import", False): groups.append("Utilities")
    return groups or ["Quotation"]


def feature_control_notes():
    show_header("Feature Control Notes", "section-master")
    st.info("Feature enable/disable is controlled from Client Master for each client. Super Admin can see all modules.")
    if is_super_admin():
        st.dataframe(load_table("clients", 1000), use_container_width=True)
...
def get_module_mapping():
    return {
        ...
        "Company Profile": company_profile,
        "Financial Year Master": financial_year_master,
        "Branch / Location Master": branch_master,
        "Department Master": department_master,
        "Cost Center Master": cost_center_master,
        "Role & Permission Master": role_permission_master,
        "Approval Role Master": approval_role_master,
        "Document Series": document_series_master,
        "Voucher Type Master": voucher_type_master,
        "Ledger Group Master": ledger_group_master,
        "Ledger Master": ledger_master,
        "Opening Balance Master": opening_balance_master,
        "Bank Master": bank_master,
        "Stock Group Master": stock_group_master,
        "Stock Ledger Master": stock_ledger_master,
        "Opening Stock Master": opening_stock_master,
        "Warehouse Master": warehouse_master,
        "HSN / SAC Master": hsn_sac_master,
        "GST Settings": gst_settings_master,
        "GST / Tax Master": tax_master,
        "TDS Section Master": tds_section_master,
        "Payment Terms Master": payment_terms_master,
        "Attendance Management": attendance,
        "IN / OUT Register": inout_register,
        "Visitor Register": visitor_register,
        "Task Delegation": task_delegation,
        ...
        "WIP Stock": stock_wip,
        "Stock Voucher": stock_voucher,
        "Stock Ledger Report": lambda: show_table_with_edit_delete("stock_ledger", load_table("stock_ledger", 5000), "Stock Ledger Report"),
        "Sales GST Invoice": sales_invoice,
        "Purchase GST Invoice": purchase_invoice,
        "Expense GST": expense_gst,
        "Service Voucher": service_voucher,
        ...
        "ERP Control Center": erp_control_center,
        "Enterprise Reports": enterprise_reports,
        "Registers / Reports": reports,
        "Audit Log": audit_log_report,
        "Import / Export / Backup": enterprise_import_export_center,
        "Calculation Book": calculation_book,
        "Quotation": quotation_module,
        "Feature Control Notes": feature_control_notes,
        "No module available": placeholder_denied,
    }
