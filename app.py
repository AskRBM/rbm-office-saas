...
def online_generic_module(module_title):
    ...
    if df.empty:
        ...
        df_show = pd.DataFrame([sample])
    else:
        df_show = df.copy()

    search_text = st.text_input(f"Search {module_title} Saved Data", key=f"search_saved_{module_title}")
    if search_text and not df_show.empty:
        mask = df_show.astype(str).apply(lambda x: x.str.contains(search_text, case=False, na=False)).any(axis=1)
        df_show = df_show[mask]

    st.dataframe(df_show, use_container_width=True)

    if not df_show.empty:
        labels = [f"Row {i} - " + " | ".join([str(v) for v in df_show.iloc[i].astype(str).head(3).tolist()]) for i in range(len(df_show))]
        sel_label = st.selectbox("Select Saved Record for Print / Edit / Delete", labels, key=f"select_saved_{module_title}")
        sel_pos = labels.index(sel_label) if sel_label in labels else 0
        selected_df = df_show.iloc[[sel_pos]].copy()
    else:
        selected_df = df_show.copy()
        sel_pos = None

    b1,b2,b3,b4,b5 = st.columns(5)
    b1.download_button("Export CSV", df_show.to_csv(index=False).encode(), file_name=f"{module_title.replace(' ','_')}.csv", mime="text/csv", use_container_width=True)
    b2.download_button("Export Excel", to_excel_bytes(df_show), file_name=f"{module_title.replace(' ','_')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    print_clicked = b3.button("Print Preview / PDF", use_container_width=True, key=f"pp_btn_{module_title}_fixed")
    edit_clicked = b4.button("Edit Selected", use_container_width=True, key=f"edit_btn_{module_title}_fixed")
    delete_clicked = b5.button("Delete Selected", use_container_width=True, key=f"del_btn_{module_title}_fixed")

    if print_clicked:
        st.session_state[f"show_print_{module_title}"] = True
    if edit_clicked and sel_pos is not None:
        st.session_state[f"edit_selected_{module_title}"] = selected_df.iloc[0].to_dict()
        st.info("Selected record loaded below for review. For full edit, change fields above and save again.")
        st.json(st.session_state[f"edit_selected_{module_title}"])
    if delete_clicked and sel_pos is not None:
        # Safe delete from current session storage. Supabase generic table delete needs id column; if absent, this does not crash.
        try:
            all_rows = st.session_state.get(_SPECIAL_STORAGE_KEY, [])
            target = selected_df.iloc[0].to_dict()
            for idx, r in enumerate(list(all_rows)):
                if r.get("module_name") == module_title and all(str(r.get(k,'')) == str(target.get(k,'')) for k in target.keys() if k in r):
                    all_rows.pop(idx); break
            st.session_state[_SPECIAL_STORAGE_KEY] = all_rows
            st.success("Selected saved record deleted from current session list.")
            st.rerun()
        except Exception as e:
            st.warning(f"Delete could not complete: {e}")

    if st.session_state.get(f"show_print_{module_title}"):
        _generic_print_preview_ui(module_title, selected_df if not selected_df.empty else df_show)
